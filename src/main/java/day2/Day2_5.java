...
public class Day2_5 {
  ...
  /*  문제 설명
    ...
    private static String solution(String my_string, String overwrite_string, int s) {
        ...
        // 1 ≤ overwrite_string의 길이 ≤ my_string의 길이 ≤ 1,000
        // 0 ≤ s ≤ my_string의 길이 - overwrite_string의 길이
        if(overwrite_string.length() < 1 || overwrite_string.length() > my_string.length() || my_string.length() < 1 || my_string.length() > 1000 || s < 0 || s > my_string.length() - overwrite_string.length()) {
            return "1 ≤ overwrite_string의 길이 ≤ my_string의 길이 ≤ 1,000\n0 ≤ s ≤ my_string의 길이 - overwrite_string의 길이";
        }

        // my_string에서 인덱스 s부터 overwrite_string의 길이만큼에 해당하는 부분은 "11oWor1"이고 이를 "lloWorl"로 바꾼 "HelloWorld"를 return
